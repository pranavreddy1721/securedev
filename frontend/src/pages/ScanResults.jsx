...
export default function ScanResults() {
  ...
  if (scan.status === 'queued' || scan.status === 'running') {
    return (
      <div className="mx-auto max-w-xl px-6 py-24 text-center">
        ...
        <h2 className="mt-4 text-lg font-semibold text-ink dark:text-ink-dark">
          {scan.status === 'queued' ? 'Scan queued…' : 'Scan in progress…'}
        </h2>
        <p className="mt-2 text-sm text-muted dark:text-muted-dark">
          Running npm audit, secret scanning, and Semgrep in parallel. This usually takes 30–90 seconds.
        </p>
      </div>
    );
  }
  ...
  if (scan.status === 'failed') {
    return (
      <div className="mx-auto max-w-xl px-6 py-24">
        <ErrorState message={scan.error || 'Scan failed for an unknown reason.'} />
      </div>
    );
  }
  ...
  Object.values(findingsByCategory).forEach((arr) =>
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink dark:text-ink-dark">Scan results</h1>
          <p className="mt-1 text-sm text-muted dark:text-muted-dark">
            Completed {scan.completedAt ? new Date(scan.completedAt).toLocaleString() : ''}
          </p>
        </div>
        <a
          href={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api'}/scans/${scan._id}/report`}
          className="flex items-center gap-1.5 rounded-lg border border-border dark:border-border-dark px-4 py-2 text-sm font-medium text-ink dark:text-ink-dark hover:bg-panel dark:hover:bg-panel-dark"
          target="_blank"
          rel="noreferrer"
        >
      ...
      <div className="mt-6 flex flex-wrap gap-3 text-xs text-muted dark:text-muted-dark">
        ...
        <EngineChip label="Secret scanner" status={scan.engineStatus?.secretScanner} />
        <EngineChip label="Semgrep" status={scan.engineStatus?.semgrep} />
      </div>
      ...
      {Object.keys(findingsByCategory).length === 0 && (
        <p className="mt-4 text-sm text-muted dark:text-muted-dark">No findings detected in this scan. 🎉</p>
      )}

      <div className="mt-4 space-y-3">
        {Object.entries(findingsByCategory).map(([category, findings]) => (
          <CategorySection key={category} category={category} findings={findings} />
        ))}
      </div>
    </div>
  );
...
function EngineChip({ label, status }) {
  const color =
    status === 'success' ? 'text-severity-low' : status === 'skipped' ? 'text-muted dark:text-muted-dark' : 'text-severity-high';
  return (
    <span className="rounded-full border border-border dark:border-border-dark px-3 py-1">
      {label}: <span className={color}>{status || 'unknown'}</span>
    </span>
  );
}
  ...
  return (
    <div className="rounded-xl border border-border dark:border-border-dark overflow-hidden">
      <button
        ...
        className="flex w-full items-center justify-between bg-panel dark:bg-panel-dark px-4 py-3 text-left"
      >
        <div className="flex items-center gap-2">
          <span className="font-medium text-ink dark:text-ink-dark">{CATEGORY_LABELS[category]}</span>
          <span className="rounded-full bg-border dark:bg-border-dark px-2 py-0.5 text-xs text-muted dark:text-muted-dark">
            {findings.length}
          </span>
          {isHeuristic && (
            <span className="flex items-center gap-1 text-xs text-muted dark:text-muted-dark">
              <AlertCircle className="h-3 w-3" /> heuristic, not exhaustive
            </span>
          )}
        </div>
          ...
          {findings.map((f, idx) => (
            <div key={idx} className="px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-ink dark:text-ink-dark">{f.title}</span>
              </div>
              <p className="mt-1 text-sm text-muted dark:text-muted-dark">{f.description}</p>
              {f.file && (
